# @power/power-ai-skills

> 此 README 由 `node ./scripts/generate-readme.mjs` 自动生成，请不要手工修改。

企业前端 AI skill 中心仓库，面向 `@power/runtime-vue3`、`@power/p-components`、`@power/style` 以及通用前端页面骨架。

## 目标

- 统一维护企业级前端 skill
- 通过私有 npm 分发到各业务项目
- 用 `.power-ai/` 作为项目内唯一真实源目录
...
- `.power-ai/skills/`：企业公共 skill 的唯一真实源目录
- `.power-ai/skills/project-local/`：项目私有补充规则
- `.power-ai/shared/` 和 `.power-ai/adapters/`：工具共享文件与适配文件
- `.power-ai/context/`：项目治理上下文快照目录
- `.power-ai/governance/`：画像决策、会话决策、promotion trace 等治理账本目录
- `.power-ai/reports/`：baseline、doctor、治理汇总与历史查询等报告目录
- `.power-ai/proposals/`：wrapper promotion proposal 与 archive 目录
- `.power-ai/tool-registry.json`：工具注册表副本
- `.power-ai/team-policy.json`：团队策略快照副本
- `.power-ai/team-defaults.json`：团队默认接入配置副本
- `.power-ai/template-registry.json`：模板注册表副本
- `.power-ai/selected-tools.json`：项目当前实际启用的工具集合

## 工具注册表与团队默认配置

- 工具注册表：`config/tool-registry.json`
- 团队默认配置：`config/team-defaults.json`
- 模板注册表：`config/template-registry.json`
- JSON Schema：`config/schemas/tool-registry.schema.json`、`config/schemas/team-defaults.schema.json`、`config/schemas/template-registry.schema.json`
...
完整产物说明和字段契约见 `docs/command-manual.md`。
...
更多 wrapper 示例脚本、适配矩阵和完整命令参数见 `docs/tool-adapters.md` 与 `docs/command-manual.md`。
...
- `vscode-agent`：VS Code Agent Mode
...
- `shared-agents`：`shared/AGENTS.md`，ownerTool: `agents-md`
- `shared-claude`：`shared/CLAUDE.md`，ownerTool: `claude-code`
- `shared-gemini`：`shared/GEMINI.md`，ownerTool: `gemini-cli`
- `shared-conventions`：`shared/CONVENTIONS.md`，ownerTool: `aider`
- `shared-aider-config`：`shared/aider.conf.yml`，ownerTool: `aider`
- `cursor-rules`：`adapters/cursor/skills.mdc`，ownerTool: `cursor`
- `cline-rules`：`adapters/cline/01-power-ai.md`，ownerTool: `cline`
- `windsurf-rules`：`adapters/windsurf/01-power-ai.md`，ownerTool: `windsurf`
...
- `cline` -> `.clinerules/01-power-ai.md`
- `windsurf` -> `.windsurf/rules/01-power-ai.md`
...
完整发布步骤、门禁要求和发包顺序见 `docs/release-process.md`。
...
- `SKILL.md` 只保留用途、触发条件、实现清单和切换规则
- 详细模板、字段表和接口说明放到 `references/`
- 修改 skill 后必须重新生成 manifest
- 发布前执行 `pnpm release:prepare`
