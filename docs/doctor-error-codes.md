...
| `PAI-RELEASE-013` | 最近一次受控 publish 执行仍停在 `blocked` / `confirmation-required` / `acknowledgement-required` 闸口，或真实 publish 已尝试但失败 | 复核 `manifest/release-publish-record.json` 与 `manifest/release-publish-failure-summary.md`，确认后用需要的 `execute-release-publish` 参数重新推进，或先处理真实 publish 失败原因 |
